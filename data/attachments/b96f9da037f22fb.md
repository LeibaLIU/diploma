...
- Name: ui/login.spec.js >> UI · Login @UI @AUTH >> Shows error for invalid credentials
...
Locator: locator('.message-error.validation-summary-errors')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.message-error.validation-summary-errors')
            ...
            - generic [ref=e92]:
              ...
              - generic [ref=e96]:
                ...
                - generic [ref=e101]:
                  ...
                  - textbox "Email:" [active] [ref=e103]: not-a-real-user@example.com
                - generic [ref=e104]:
...
```ts
  ...
  8  |     super(page, '/login');
  9  | 
  10 |     this.email = page.locator('#Email');
  11 |     this.password = page.locator('#Password');
  12 |     this.rememberMe = page.locator('#RememberMe');
  13 |     this.submitBtn = page.locator('input.login-button');
  14 |     this.summaryError = page.locator('.message-error.validation-summary-errors');
  15 |   }
  16 | 
  17 |   async login(email, password) {
  18 |     await allure.step(`Log in as ${email}`, async () => {
  ...
  23 |   }
  24 | 
  25 |   async expectLoginError() {
  26 |     await allure.step('Expect login error visible', async () => {
> 27 |       await expect(this.summaryError).toBeVisible();
     |                                       ^ Error: expect(locator).toBeVisible() failed
  28 |       await this.attachScreenshot('Login error');
  29 |     });
  30 |   }
  31 | }
  32 | 
